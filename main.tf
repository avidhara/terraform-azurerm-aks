...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  dynamic "default_node_pool" {
    ...
    content {
      ...
      #checkov:skip=CKV_AZURE_168: MAX_PODS has been set to 110
      ...
      node_count       = default_node_pool.value.enable_auto_scaling ? null : try(default_node_pool.value.node_count, 3)
  ...
  dynamic "network_profile" {
    ...
    content {
      ...
      #checkov:skip=CKV_AZURE_7: network_policy already been set to Azure
  ...
  #checkov:skip=CKV_AZURE_170: we are using Standard (which includes the Uptime SLA)
...
resource "azurerm_kubernetes_cluster_node_pool" "this" {
  ...
  #checkov:skip=CKV_AZURE_168: MAX_PODS has been set to 50
...
resource "azurerm_role_assignment" "app_gateway" {
  count                = var.create_aks && length(var.ingress_application_gateway) > 0 ? 1 : 0
  scope                = var.vnet_id
  role_definition_name = "Network Contributor"
  principal_id         = azurerm_kubernetes_cluster.this[0].ingress_application_gateway[0].ingress_application_gateway_identity[0].object_id
}